...
export class AesManager {
    ...
    public static EncryptCBCPbkdf2Key(plantText: Buffer, pbkdf2Key: Buffer): Buffer | Error {
        if (pbkdf2Key.length !== Pbkdf2KeySize) {
            return new Error(`Crypted Key length error(${pbkdf2Key.length}), Crypted Key length should be ${Pbkdf2KeySize}`);
        }
        return this.EncryptCBC(plantText, pbkdf2Key.slice(0, Pbkdf2KeySize - 16), pbkdf2Key.slice(Pbkdf2KeySize - 16)); //加密
    }

    public static DecryptCBCPbkdf2Key(plantText: Buffer, pbkdf2Key: Buffer): Buffer | Error {
        if (pbkdf2Key.length !== Pbkdf2KeySize) {
            return new Error(`Crypted Key length error(${pbkdf2Key.length}), Crypted Key length should be ${Pbkdf2KeySize}`);
        }
    ...
    public static EncryptCBC(plantText: Buffer, key: Buffer, iv: Buffer): Buffer | Error {
        if (iv.length !== 16) {
            return new Error(`VI length error(${iv.length}), aes cbc IV length should be 16`);
        }
    ...
    public static DecryptCBC(ciphertext: Buffer, key: Buffer, iv: Buffer): Buffer| Error {
        if (iv.length !== 16) {
            return new Error(`VI length error(${iv.length}), aes cbc IV length should be 16`);
        }
        try {
